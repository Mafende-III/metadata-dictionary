...
export default function DashboardsPage() {
  ...
  const [testSqlViewId, setTestSqlViewId] = useState('YOgvi13YgKJ'); // Default dashboards SQL view
  ...
  // Use metadata hook to fetch dashboards
  const {
    metadata,
    isLoading,
    error,
    ...
    updateFilters,
    fetchMetadata,
  } = useMetadata(METADATA_TYPES.DASHBOARD, session, filters);
  ...
  useEffect(() => {
    const fetchGroups = async () => {
      ...
      try {
        setGroupOptions([
          { id: 'group1', name: 'Executive Dashboards' },
          { id: 'group2', name: 'Operational Dashboards' },
          { id: 'group3', name: 'Financial Dashboards' },
          { id: 'group4', name: 'Clinical Dashboards' },
        ]);
      } catch (error) {
  ...
  const handleExport = (format: ExportFormat, includeQuality: boolean) => {
    ...
    const url = ExportService.createDownloadLink(
      exportContent,
      `dashboards_export_${new Date().toISOString().split('T')[0]}`,
      format
    );
    ...
    a.href = url;
    a.download = `dashboards_export_${new Date().toISOString().split('T')[0]}.${format}`;
        ...
        {activeTab === 'debug' && (
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <SqlViewDataDisplay category="dashboards" />
            </div>
          </div>
        )}
                ...
                <div className="flex space-x-4 items-end">
                  <div className="flex-1">
                    ...
                    <input
                      ...
                      placeholder="Enter SQL View UID for dashboards"
                    />
                    <p className="text-xs text-blue-600 mt-1">
                      Default: YOgvi13YgKJ (DHIS2 demo dashboards view)
                    </p>
                  </div>
                  <button
              ...
              <div className="border border-gray-200 rounded-lg">
                <div className="bg-gray-50 px-4 py-3 border-b border-gray-200">
                  <h3 className="font-medium text-gray-900">
                    🚀 Enhanced Multi-Page Dashboards Analysis
                  </h3>
