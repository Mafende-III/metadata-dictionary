...
CREATE TABLE IF NOT EXISTS dhis2_instances (
  ...
  CONSTRAINT unique_instance_credentials UNIQUE(base_url, username)
);
...
DROP POLICY IF EXISTS "Allow all operations for now" ON sql_view_templates;
DROP POLICY IF EXISTS "Allow all operations for now" ON sql_view_executions;
DROP POLICY IF EXISTS "Allow all operations for now" ON user_sessions;
DROP POLICY IF EXISTS "Allow all operations for now" ON processing_queue;

-- Create policies (removed IF NOT EXISTS since it's not supported)
...
CREATE POLICY "Allow all operations for now" ON sql_view_templates FOR ALL USING (true);
CREATE POLICY "Allow all operations for now" ON sql_view_executions FOR ALL USING (true);
CREATE POLICY "Allow all operations for now" ON user_sessions FOR ALL USING (true);
CREATE POLICY "Allow all operations for now" ON processing_queue FOR ALL USING (true);
...
DROP TRIGGER IF EXISTS update_sql_view_templates_updated_at ON sql_view_templates;
...
-- Insert some sample data (using DO blocks to handle conflicts properly)
DO $$
BEGIN
    -- Insert sample DHIS2 instances
...
EXCEPTION WHEN OTHERS THEN
    -- If there are any conflicts or errors, just continue
    NULL;
END $$;
